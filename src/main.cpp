...
#if defined(NDEBUG)
# error "Ioniccoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x74fc52a23cecd1b2f53ee27c4706dfd76d792fc9f19b5e0b545ec829f8143d7f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Ioniccoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Ioniccoin Signed Message:\n";
...
{
    // Ioniccoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Ioniccoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Ioniccoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Ioniccoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Ioniccoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Ioniccoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xda1ba8ee5d1c6439aca895b569c39f7721f20f961ed06e6a8e1b9342198280ca");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is time for IonicCoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0435a285a922b4552b58d42cdd669580cb85f0c85efd5121727f0523b49c9996fb3934bd510309c355606f7a426cabe962876bf572bd8637b110f9f0e53d9e9e98") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1524656331;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084666148;

        if (fTestNet)
        {
            block.nTime    = 1524656316;
            block.nNonce   = 386372199;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xdbee14721e1f30a83ec4cbc6b6b280f16bd35c17025575441220463594dba522"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf6, 0xc6, 0xb6, 0xd6 }; // Ioniccoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("IoniccoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("IoniccoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static IoniccoinMiner(CWallet *pwallet)
{
    printf("IoniccoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ioniccoin-miner");
    ...
    try { loop {
        ...
        printf("Running IoniccoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("IoniccoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&IoniccoinMiner, pwallet));
}
